...
void Cylinder::normal_vector(double points[9], double normal[9]){
    for(int i =0;i<3;i++){
        double x=points[3*i];
        ...
        if((y==0.5 || y==-0.5)&&r==0.5){
            //use cross product to determine values
            double x1=points[0];
            double x2=points[3];
            double x3=points[6];
            double z1=points[2];
...
void Cylinder::tesselate(int p1, int p2){
    if (m_triangles){
        delete[] m_triangles;
    }
    if(p2<3){
